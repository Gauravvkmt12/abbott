...
<body>
    <div class="conatiner bg-white">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item mt-2"><a href="index.php"
                                    ...
                                    <div class="scrollable-content">
                                        <div class="row">
                                            <div class="col mb-3 mb-md-0">
                                                <h5 class="filter-title">FILTER "<span id="selectedTherapy">
                                                        <?= isset($_GET['therapy_name']) ? htmlspecialchars($_GET['therapy_name']) : '' ?>
                                                    </span>"</h5>
                                                <div class="therpy-gri">
                                                    <?php foreach ($therapies as $therapy): ?>
                                                    <div class='therapy-name'>
                                                        <a
                                                            href='therpy.php?therapy_id=<?= $therapy["id"] ?>&therapy_name=<?= urlencode($therapy["name"]) ?>'>
                            ...
                            <div class="accordion-item mb-3">
                                ...
                                <div id="collapse<?= $index + 1 ?>" class="accordion-collapse collapse"
                                    ...
                                    <div class="accordion-body">
                                        <b>PRODUCT DESCRPTION:</b>
                                        <?= htmlspecialchars($product['product_descrption']) ?><br>
                                        <b>BUSINESS:</b>
                                        <?= htmlspecialchars($product['Business']) ?><br>
                                        <b>MOLECULE:</b>
                                        <?= htmlspecialchars($product['MOLECULE']) ?><br>
                                        <b>FORM:</b>
                                        <?= htmlspecialchars($product['FORM']) ?><br>
                                        <b>STRENGTH:</b>
                                        <?= htmlspecialchars($product['STRENGTH']) ?><br>
                                        <b>BUSINESS AREAS:</b>
                                        <?= htmlspecialchars($product['BUSINESS_AREAS']) ?><br>
                                        <b>THERAPY AREAS:</b>
                                        <?= htmlspecialchars($product['therpy_name']) ?><br>
                                    </div>
                                </div>
                            </div>
                            <?php else: ?>
                            <div class="accordion-item">
                                <h2 class="accordion-header" id="heading<?= $index + 1 ?>">
                                    <button class="accordion-button" type="button"
                                        onclick="window.location.href='<?= htmlspecialchars($product['product_link']) ?>'">
                                        <?= htmlspecialchars($product['product_name']) ?>
                                    </button>
                                </h2>
                            </div>
